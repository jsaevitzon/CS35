...
//Global declerations
scene_t scene;
float  print_color[width][height][3];
//need this global for the for loop
int nthreads;


void* threadMe(void* num)
{
  ...
  const double subsample_dx
	...
	: pixel_dy;
      ...
      /*We are going to split up the loops so that 
	each thread does a chunk of the work */
      ...
      /* for every pixel */
      for( int px=tid; px<width; px+=nthreads )
	{
	  ...
	  for( int py=0; py<height; ++py )
		...
		scaled_color[i] = max( min(scaled_color[i], 255), 0);
	      ...
	      //save to print color so i can print later
	      //for(int i=0; i<3; i++)
		print_color[px][py][0] = scaled_color[0];
		print_color[px][py][1] = scaled_color[1];
		print_color[px][py][2] = scaled_color[2];
...
int main( int argc, char **argv )
{
  ...
  if(nthreads<1)
    {
      fprintf(stderr, "Need at least 1 thread, you wrote: %d", nthreads);
      exit(-1);
    }
  
  
  /*only need to do all this once*/
  ...
  for(i=0; i<nthreads; i++)
    {
      int ret = pthread_create(&th[i], NULL, threadMe, (void *) i);
      if(ret)
	{
  ...
  for( int px=0; px<width; ++px )
    {
      for( int py=0; py<height; ++py )
	printf( "%.0f %.0f %.0f\n",
		print_color[px][py][0], print_color[px][py][1], print_color[px][py][2] ); 
      printf("\n");
    }
